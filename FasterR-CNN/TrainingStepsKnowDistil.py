...
def train_step_know_distil(student, optimizer, data_loader, device, epoch, max_epoch, iteration, print_freq,
               scaler=None, profiler=None, teacher=None, adaptive_model=None):
    ...
    start_weight = 0.1
    end_weight = 1
    hint_weight = start_weight + (end_weight - start_weight) * (float(epoch) / max_epoch)
    print(start_weight)
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with ((torch.amp.autocast('cuda', enabled=scaler is not None))):
            ...
            with torch.no_grad():
                ...
                _, teacher_loss_dict, teacher_logits, teacher_features, teacher_roi_gt_targets, teacher_rpn_gt_targets = teacher(images, targets)
                #print("teacher_logits:", teacher_logits.keys())
                #print(teacher.backbone)

            student_loss_dict, student_logits, student_features, student_roi_gt_targets, student_rpn_gt_targets = student(images, targets)
            #print(student_features.shape)
            #print(teacher_features.shape)
            adapted_student_features = adaptive_model(student_features)
            #print(adapted_student_features.shape)
            #print("student_logits: ", student_logits.keys())

        student_losses = sum(loss for loss in student_loss_dict.values())
        #print("studen loss: ", student_losses)
        ...
        # KNOWLEGE DISTIL
        object_loss, class_loss = get_distil_loss(device, student_logits, teacher_logits, loss_dict_reduced['loss_classifier'].item(), loss_dict_reduced['loss_box_reg'].item(),
                        loss_dict_reduced['loss_objectness'].item(), loss_dict_reduced['loss_rpn_box_reg'].item(), teacher_roi_gt_targets, teacher_rpn_gt_targets,
                                      student_roi_gt_targets, student_rpn_gt_targets, teacher_features, adapted_student_features, epoch, max_epoch)

        #print(f"class loss: {class_loss} | objectness loss: {object_loss}")
        if epoch > 1:
            student_loss_dict["loss_objectness"] = object_loss
            student_loss_dict["loss_classifier"] = object_loss
        student_losses = sum(loss for loss in student_loss_dict.values())
        #for key, loss in student_loss_dict.items():
            #print(f"Key: {key}, Loss: {loss}")
        #print("studen updated: ", student_losses)
...
def regression_distil(device, bbox_smooth_loss, student_bbox_delta, teacher_bbox_delta,
                      student_targets, teacher_targets, margin=0.01, penalty=1):
...
def classification_distil(student_logits, teacher_logits, weight_factor, student_loss, temperature=10):
    ...
    # Compute soft loss (knowledge distillation loss)
    epsilon = 1e-10
    soft_loss = -torch.mean(teacher_probs * torch.log(student_probs + epsilon))
