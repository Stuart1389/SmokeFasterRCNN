...
from ..resnet import (resnet50, ResNet50_Weights, resnet101, ResNet101_Weights,
                      resnet18, ResNet18_Weights, resnet34, ResNet34_Weights, resnet152, ResNet152_Weights)
...
def fasterrcnn_resnet50_fpn_v2(
    *,
    ...
    progress: bool = True,
    num_classes: Optional[int] = None,
    anchor_values: Optional[tuple] = None,
    anchor_ratios: Optional[tuple] = None,
    ...
    **kwargs: Any,
    ...
    weights_backbone = ResNet50_Weights.verify(weights_backbone)
    #print(anchor_values, anchor_ratios)
    ...
    if(anchor_values != None and anchor_ratios != None):
        rpn_anchor_generator = AnchorGenerator(anchor_values, anchor_ratios)
    else:
        rpn_anchor_generator = _default_anchorgen()
    ...
    if weights is not None:
        model_weights = weights.get_state_dict(progress=progress, check_hash=True)
        ...
        """
        model_weights["rpn.head.cls_logits.weight"] = expand_weights(model_weights["rpn.head.cls_logits.weight"],
                                                                     (6, 256, 1, 1))
        model_weights["rpn.head.cls_logits.bias"] = expand_bias(model_weights["rpn.head.cls_logits.bias"], (6,))
        model_weights["rpn.head.bbox_pred.weight"] = expand_weights(model_weights["rpn.head.bbox_pred.weight"],
                                                                    (24, 256, 1, 1))
        model_weights["rpn.head.bbox_pred.bias"] = expand_bias(model_weights["rpn.head.bbox_pred.bias"], (24,))
        model.load_state_dict(model_weights, strict=True)
        """

        if (anchor_values != None and anchor_ratios != None):
            # init model rpn weights
            model.rpn.head.bbox_pred.bias.data.zero_()
            model.rpn.head.cls_logits.bias.data.zero_()
            init.normal_(model.rpn.head.bbox_pred.weight, mean=0, std=0.01)
            init.normal_(model.rpn.head.cls_logits.weight, mean=0, std=0.01)


    return model

def expand_weights(original_weight, target_shape):
    repeat_factor = target_shape[0] // original_weight.shape[0]
    return original_weight.repeat(repeat_factor, 1, 1, 1)

def expand_bias(original_bias, target_shape):
    repeat_factor = target_shape[0] // original_bias.shape[0]
    return original_bias.repeat(repeat_factor)


@register_model()
def stus_resnet_fpnv2_builder(
    *,
    roi_head_weights: Optional[dict] = None,
    rpn_weights: Optional[dict] = None,
    fpn_weights: Optional[dict] = None,
    body_weights: Optional[dict] = None,
    progress: bool = True,
    model_backbone: Optional[str] = "resnet101",
    ...
    **kwargs: Any,
    ...
    if(setTrainValues("load_coco_weights")):
        model.roi_heads.load_state_dict(roi_head_weights, strict=False)
        model.rpn.load_state_dict(rpn_weights, strict=False)
        model.backbone.fpn.load_state_dict(fpn_weights, strict=False)
        model.backbone.body.load_state_dict(body_weights, strict=False)
