...
class FeaturePyramidNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.return_layers_p = self.return_layers
        if(setTrainValues("backbone_to_use") == "default"):
            self.return_layers_p['layer4 pool'] = 'pool'
        else:
            self.return_layers_p['FPN_generated'] = 'N/a'
        if(setTrainValues("plot_feature_maps")):
            print("FPN return layers: ",self.return_layers)
    ...
    def plt_feature_maps(self, feature_map_list, layer_name_list):
        pyramid = ["P2", "P3", "P4", "P5", "P6"] # for resnet
        ...
        for idx, (feature_map, layer_name) in enumerate(zip(feature_map_list, layer_name_list)):
            ...
            if(not setTrainValues("backbone_to_use") == "mobilenet" and not setTrainValues("backbone_to_use") == "resnet_builder"):
                ax.set_title(f"Layer: {layer_name}\nFPN level: {pyramid[idx]}", fontsize=20, fontweight="bold") # different fpn structure for dif models
    ...
    def forward(self, x: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        if(setTrainValues("plot_feature_maps")):
            ...
            for batch_idx in range(batch_size):
                batch_results = [result[batch_idx] for result in x]
                self.plt_feature_maps(batch_results, self.return_layers)
        ...
        #print("whwhwh", names, len(results))
        #print("result", results[4].shape)
        if (setTrainValues("plot_feature_maps")):
            batch_size = results[0].shape[0]
            for batch_idx in range(batch_size):
                batch_results = [result[batch_idx] for result in results]
                self.plt_feature_maps(batch_results, self.return_layers_p)
